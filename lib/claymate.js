...
function Claymate() {
	...
	function validateBuildInput(buildInput) {
		...
		if (!fs.existsSync(path.join(opts.gumbyPath, 'js/libs/ui'))) {
			console.error('Path "' + opts.gumbyPath + '" does not exist or does not contain gumby.');
			process.exit(1);
		}
	...
	this.build = function(opts) {
		...
		// Master gumby script
		console.log('\tAdding file ' + path.join(opts.gumbyPath, 'js/libs/gumby.js'));
		...
		for (var i = 0; i < opts.modules.length; i++) {
			...
			if (fs.existsSync(tmp2)) {
				...
				if (fs.existsSync(modulePathGumby)) {
					console.log('\tAdding file ' + modulePathGumby);
					files.push(modulePathGumby);
				} else if (fs.existsSync(modulePathjQuery)) {
					console.log('\tAdding file ' + modulePathjQuery);
					files.push(modulePathjQuery);
				} else {
					console.warn('\tWARNING: Unable to find core module corresponding to ' + tmp);
				}
		...
		for (var j = 0; j < opts.addons.length; j++) {
			...
			if (fs.existsSync(opts.addons[j])) {
				console.log('\tAdding file ' + opts.addons[j]);
				files.push(opts.addons[j]);
			} else {
				console.warn('\tWARNING: Unable to find external module ' + tmp);
			}

		}

		// Final init script
		console.log('\tAdding file ' + path.join(opts.gumbyPath, 'js/libs/gumby.init.js'));
		...
		for (var h = 0; h < opts.files.length; h++) {
			...
			if (fs.existsSync(opts.files[h])) {
				console.log('\tAdding file ' + opts.files[h]);
				files.push(opts.files[h]);
			} else {
				console.warn('\tWARNING: Unable to find external file ' + tmp);
			}
		...
		if (opts.buildSourceMap) {
			...
			console.log('\tAdding sourcemap ' + path.join(opts.outPath, 'gumby.js.map'));
		...
		console.log('\n\tFinished generating ' + path.join(opts.outPath, 'gumby.min.js'));
		...
		if (!fs.existsSync(path.join(workingDir, '/gumby.json')) && !opts.quiet) {
			...
			console.log(message);
			console.log(JSON.stringify(printOpts, null, "\t"));
