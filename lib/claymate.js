...
function Claymate() {
	...
	this.build = function(modules, gumbyPath) {
		...
		for (var i = 0; i < modules.length; i++) {
			files.push(path.join(gumbyPath, 'js/libs/ui/', 'gumby.' + path.basename(modules[i], '.js') + '.js'));
		}
		...
		fs.writeFile(path.join(gumbyPath, 'js/libs/gumby.min.js'), minified.code, writeErrorHandler);
		fs.writeFile(path.join(gumbyPath, 'js/libs/gumby.js.map'), minified.map, writeErrorHandler);
