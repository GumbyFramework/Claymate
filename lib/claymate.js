...
function Claymate() {
	...
	function validateBuildInput(buildInput) {
		...
		if (util.isArray(buildInput.extraFiles)) {

			opts.extraFiles = buildInput.extraFiles;
	...
	this.build = function(opts) {
		...
		for (var h = 0; h < opts.extraFiles.length; h++) {

			if (fs.existsSync(opts.extraFiles[h])) {
				files.push(opts.extraFiles[h]);
			} else {
		...
		if (!fs.existsSync(path.join(workingDir, '/gumby.json'))) {
